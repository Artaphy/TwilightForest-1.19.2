...
public class Hydra extends Mob implements Enemy, EnforcedHomePoint {
	...
	private static final int MAX_BLOCKS_TO_DESTROY_PER_TICK = 64; // 性能防护：每tick最多破坏64个方块
	private static final int MAX_AABB_VOLUME = 1000; // 性能防护：AABB体积阈值，超过则跳过
	...
	private void destroyBlocksInAABB(AABB box) {
		if (this.deathTime <= 0 && ForgeEventFactory.getMobGriefingEvent(this.getLevel(), this)) {
			// 性能防护：AABB体积过大直接跳过
			double volume = (box.maxX - box.minX + 1) * (box.maxY - box.minY + 1) * (box.maxZ - box.minZ + 1);
			if (volume > MAX_AABB_VOLUME) {
				return;
			}
			int destroyed = 0;
			for (BlockPos pos : WorldUtil.getAllInBB(box)) {
				if (EntityUtil.canDestroyBlock(this.getLevel(), pos, this)) {
					...
					destroyed++;
					if (destroyed >= MAX_BLOCKS_TO_DESTROY_PER_TICK) {
						break;
					}
				}
			}
